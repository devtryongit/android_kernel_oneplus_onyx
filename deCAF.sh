...
# deCAF Kernel Universal Build Script
#
...
step3_compile()
{
	...
	if [ "y" == "$COMPILE_DTB" ]; then
		...
		chmod 777 tools_deCAF/$DTBTOOL
		tools_deCAF/$DTBTOOL $DTBTOOL_CMD -o $BUILD_PATH/$OUTPUT_FOLDER/arch/$ARCHITECTURE/boot/dt.img -s 2048 -p $BUILD_PATH/$OUTPUT_FOLDER/scripts/dtc/ $BUILD_PATH/$OUTPUT_FOLDER/arch/$ARCHITECTURE/boot/
	fi
...
step4_prepare_anykernel()
{
	...
	cd $REPACK_PATH
	cp -R $BUILD_PATH/anykernel_deCAF/* .
...

	{
		...
		if [ "$(ls -A $BUILD_PATH/modules_deCAF)" ]; then
			cp $BUILD_PATH/modules_deCAF/* $MODULES_PATH
			cd $MODULES_PATH
...
step5_create_anykernel_zip()
{
	...
	# create zip file
	cd $REPACK_PATH
	zip -r9 $deCAF_FILENAME.recovery.zip * -x $deCAF_FILENAME.recovery.zip

	# sign recovery zip if there are keys available
	if [ -f "$BUILD_PATH/tools_deCAF/testkey.x509.pem" ]; then
		echo -e ">>> signing recovery zip\n"
		java -jar $BUILD_PATH/tools_deCAF/signapk.jar -w $BUILD_PATH/tools_deCAF/testkey.x509.pem $BUILD_PATH/tools_deCAF/testkey.pk8 $deCAF_FILENAME.recovery.zip $deCAF_FILENAME.recovery.zip_signed
		cp $deCAF_FILENAME.recovery.zip_signed $deCAF_FILENAME.recovery.zip
		rm $deCAF_FILENAME.recovery.zip_signed
	fi

	md5sum $deCAF_FILENAME.recovery.zip > $deCAF_FILENAME.recovery.zip.md5

	# Creating additional files for load&flash
	echo -e ">>> create load&flash files\n"

	cp $deCAF_FILENAME.recovery.zip cm-kernel.zip
...
step8_transfer_kernel()
{
	...
	if [ ! -z "$SMB_SHARE_KERNEL" ]; then
		smbclient $SMB_SHARE_KERNEL -U $SMB_AUTH_KERNEL -c "mkdir $SMB_FOLDER_KERNEL\\$deCAF_VERSION"
		smbclient $SMB_SHARE_KERNEL -U $SMB_AUTH_KERNEL -c "put $REPACK_PATH/$deCAF_FILENAME.recovery.zip $SMB_FOLDER_KERNEL\\$deCAF_VERSION\\$deCAF_FILENAME.recovery.zip"
		smbclient $SMB_SHARE_KERNEL -U $SMB_AUTH_KERNEL -c "put $REPACK_PATH/$deCAF_FILENAME.recovery.zip.md5 $SMB_FOLDER_KERNEL\\$deCAF_VERSION\\$deCAF_FILENAME.recovery.zip.md5"
		smbclient $SMB_SHARE_KERNEL -U $SMB_AUTH_KERNEL -c "put $REPACK_PATH/checksum $SMB_FOLDER_KERNEL\\$deCAF_VERSION\\checksum"
		smbclient $SMB_SHARE_KERNEL -U $SMB_AUTH_KERNEL -c "put $REPACK_PATH/cm-kernel.zip $SMB_FOLDER_KERNEL\\$deCAF_VERSION\\cm-kernel.zip"
		return
	fi
	...
	if [ ! -z "$SSH_FTP_REMOTE" ]; then
		...
		mkdir -p ~/bbuild_transfer/$deCAF_VERSION

		cp $REPACK_PATH/$deCAF_FILENAME.recovery.zip ~/bbuild_transfer/$deCAF_VERSION
		cp $REPACK_PATH/$deCAF_FILENAME.recovery.zip.md5 ~/bbuild_transfer/$deCAF_VERSION
		cp $REPACK_PATH/checksum ~/bbuild_transfer/$deCAF_VERSION
		cp $REPACK_PATH/cm-kernel.zip ~/bbuild_transfer/$deCAF_VERSION
...
step9_send_finished_mail()
{
	...
	if [ -z "$FINISH_MAIL_TO" ]; then
		...
		cat $ROOT_PATH/compile.log | /usr/bin/mailx -s "Compilation for $KERNEL_NAME $deCAF_VERSION finished!!!" $FINISH_MAIL_TO
	fi
}
...
display_help()
{
	...
	echo "  Kernel Filename: $deCAF_FILENAME"
	echo
	echo "======================================================================"
}
